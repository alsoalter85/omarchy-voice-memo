...
- **AI cleanup**: the transcript is piped through the configured CLI (`pi --print` by default; legacy Claude/Codex presets remain available) with a cleanup prompt; empty/failed output simply falls back to the raw transcript.
...
Pi must be installed and authenticated for the selected provider. Check with
`pi auth check --provider openrouter`. Cleanup sends transcript text to the
provider; audio transcription stays local. Existing configs selecting `claude`
keep that engine until changed to `VOICE_MEMO_AI_ENGINE=pi`. Clear any old
`VOICE_MEMO_AI_CMD` override and set the provider/model as in `config.example`.
Pi failures/timeouts discard partial output and retain the raw transcript.
When switching back to a legacy engine, also unset `VOICE_MEMO_AI_MODEL` to
use that engine's default, or set a model supported by that engine.
